...
inspectItems monkey = monkey { items = items'', inspected = inspected' }
    where items' = map (operation monkey) $ items monkey
          items'' = map (`div` 3) items'
...
turn :: Int -> MonkeyMap -> MonkeyMap
turn monkeyNumber monkeys = case items monkey of
    [] -> monkeys
    items -> monkeys'
    where monkey = get monkeys monkeyNumber
          monkey' = inspectItems monkey
          monkeys' = throwItems monkey' monkeys

turnAll :: Int -> MonkeyMap -> MonkeyMap
turnAll monkeyNumber monkeys
    | monkeyNumber == Map.size monkeys = monkeys
    | otherwise = turnAll (monkeyNumber+1) monkeys'
    where monkeys' = turn monkeyNumber monkeys

processRound :: MonkeyMap -> MonkeyMap
processRound monkeys = turnAll 0 monkeys

rounds :: Int -> MonkeyMap -> MonkeyMap
rounds n monkeys
    | n == 0    = monkeys
    | otherwise = rounds (n-1) monkeys'
    where monkeys' = processRound monkeys
...
main = do
    ...
    let monkeys = toMap $ map parseMonkey $ partitionOn "" $ lines $ c
    let monkeys' = rounds 20 monkeys
    putStrLn "Input:"
