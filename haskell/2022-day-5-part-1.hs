...
applyProcedure :: (String, Int) -> Char -> Int -> Int -> String
applyProcedure (stack, number) fromChar from to
    | from == number = tail stack
    | to == number   = fromChar : stack
    | otherwise      = stack

move :: [String] -> Int -> Int -> [String]
move stacks from to = 
    let withIndices = zip stacks [1..]
    in [applyProcedure tup (head (stacks !! (from-1))) from to | tup <- withIndices]

-- move 2 from 4 to 9
-- (2,4,9)
process :: [String] -> (Int, Int, Int) -> [String]
process stacks (0, _, _) = stacks
process stacks (1, from, to) = move stacks from to
process stacks (n, from, to) = process (move stacks from to) ((n-1), from, to)
